...
<div id="bookmarks" hx-get="{{ route('dashboard') }}" hx-include="#filters" hx-vals='{"view_type": "{{ request('
    ...
    <div class="columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4">
        @foreach ($bookmarks as $bookmark)
        <div
            class="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden border border-gray-100 break-inside-avoid mb-4">
            <div class="flex items-center p-4 border-b border-gray-100">
                ...
                <div class="truncate flex-grow">
                    ...
                    <span class="text-xs text-gray-500 truncate block">{{ parse_url($bookmark->url, PHP_URL_HOST)
                        }}</span>
                </div>
            </div>
            @if ($bookmark->description)
            ...
            <div class="px-4 py-2">
                <div class="flex flex-wrap gap-1">
                    ...
                    <a style="background-color: {{ $tag->background_color }}; color: {{ $tag->text_color }}"
                        class="inline-block text-blue-800 text-xs px-2 py-1 rounded cursor-pointer"
            ...
            <div class="p-3 bg-gray-50 flex items-center justify-between text-xs text-gray-500">
                ...
                <div class="flex items-center space-x-1">
                    <button type="button" hx-get="{{ route('bookmarks.edit', $bookmark->id) }}" hx-push-url="false"
                        hx-swap="innerHTML" hx-target="#dialog" class="p-1 rounded hover:bg-gray-200">
                        <x-icons.edit />
                    </button>
                    <button type="button" hx-get="{{ route('bookmarks.delete', $bookmark->id) }}" hx-push-url="false"
                        hx-swap="innerHTML" hx-target="#dialog" class="p-1 rounded hover:bg-gray-200">
